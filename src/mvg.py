...
_station_name = '/location/queryWeb?q={name}'
_station_id = '/location/query?q={id}'
...
def get_ids_for_satation(name):
    ...
    station_ids = [l['id'] for l in locations['locations'] if l['type'] != 'address']
    station_names = [l['name'] for l in locations['locations'] if l['type'] != 'address']
    return dict(zip(station_ids, station_names))

def get_departures_for_id(id):
    # Querying departures
